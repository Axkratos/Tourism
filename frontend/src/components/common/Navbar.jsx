...
export const Navbar = () => {
  ...
  const { authUser, setAuthUser } = useAuthContext(); // Use the authUser state and setAuthUser function from context

  const handleLogout = () => {
    // Perform logout actions: clear localStorage, reset authUser state
    localStorage.removeItem('kusmos'); // Replace with your localStorage key
    setAuthUser(null);
    navigate('/');
  };
        ...
        <div className="flex justify-between gap-7 md:order-2 space-x-3 md:space-x-0 rtl:space-x-reverse">
          ...
          {authUser ? (
            <>
              ...
              {/* Additional authenticated user links can be added here */}
            </>
          ) : (
            <>
              {/* Non-authenticated links/buttons */}
              <button
                type="button"
                className="text-white bg-red-300 hover:bg-red-400 focus:ring-2 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm px-4 py-2 text-center dark:bg-red-600 dark:hover:bg-red-700 dark:focus:ring-red-700"
